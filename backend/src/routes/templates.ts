...
// ---------------------------------------------------------------------------
// Default seed templates â€” extracted so they can be called on-demand from GET /
// ---------------------------------------------------------------------------
const SEED_TEMPLATES = [
    {
      ...
      subject: 'Action Required: Missing Credentials â€“ {{candidate_name}}',
    ...
    // â”€â”€ HR / Onboarding Document Templates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    {
      name: 'Offer Letter',
      type: 'document_request',
      category: 'hr',
      subject: 'Your Offer Letter â€“ {{position_title}} at {{agency_name}}',
    ...
    },
    {
      ...
      type: 'compliance_request',
      category: 'credentialing',
      subject: 'BLS Certification Renewal Required â€“ Expires {{expiry_date}}',
    ...
    },
    {
      ...
      type: 'compliance_request',
      category: 'compliance',
      subject: 'Incident Report Filed â€“ {{incident_date}} â€“ {{candidate_name}}',
...
// Try seeding at startup (best effort â€” table may not exist yet)
runSeedTemplates().catch(() => { /* silently skip if table missing */ });

// ---------------------------------------------------------------------------
// GET /types/list â€” must be defined before /:id to avoid route conflict
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// GET /suggest â€” AI template suggestion
// ---------------------------------------------------------------------------
router.get('/suggest', requireAuth, requirePermission('templates_view'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    // No matches â€” ask Claude which template type would work best
...
Return ONLY a valid JSON object â€” no prose, no markdown fences.

REQUIRED FORMAT:
{
  "recommended_type": "<type from the list above>",
  "recommended_category": "<category from the list above>",
  "reason": "Short explanation of why this type/category fits (1â€“2 sentences)"
}`;
...
router.get('/', requireAuth, requirePermission('templates_view'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (totalCount === 0) {
      // Table exists but is empty â€” seed defaults now
      console.log('[templates] Table empty, seeding defaultsâ€¦');
...
// ---------------------------------------------------------------------------
// GET /:id â€” get one template + version history
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// PUT /:id â€” update template (saves old version first)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// POST /:id/use â€” increment use_count
// ---------------------------------------------------------------------------
