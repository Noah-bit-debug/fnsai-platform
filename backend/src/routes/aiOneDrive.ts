...
import { filterFileResults, isFileAccessAllowed } from '../services/permissions/aiGuard';
...
// ─── Helper: get the user's permission set for filtering results ───────
async function getPermSet(req: Request): Promise<Set<string>> {
  const auth = getAuth(req);
  const dbUserId = await resolveDbUserIdFromOid(auth?.userId);
  if (!dbUserId) return new Set();
  const { permissions } = await resolveUserPermissions(dbUserId);
  return permissions;
}

// ─── GET /api/v1/ai-onedrive/folders ────────────────────────────────────
// Requires ai.search.sharepoint. Results filtered by file path rules
// so users only see folders they're allowed to open.
router.get('/folders', requireAuth, requirePermission('ai.search.sharepoint'), async (req: Request, res: Response) => {
  const path = (req.query.path as string) || '/';
  const perms = await getPermSet(req);

  try {
    const folders = await listOneDriveFolders(path);
    // Filter: only folders the user's role allows
    const filtered = folders.filter((f: any) =>
      isFileAccessAllowed(f.path ?? f.name ?? '', perms)
    );
    res.json({ folders: filtered, path, filtered_count: folders.length - filtered.length });
  } catch (err: any) {
    // Even the offline fallback goes through the filter.
    const offline = KNOWN_FOLDERS
      .map(name => ({ name, path: `/${name}`, item_count: null, offline: true }))
      .filter((f: any) => isFileAccessAllowed(f.path, perms));
    res.json({ folders: offline, path, offline: true, error: err.message });
  }
});

// ─── GET /api/v1/ai-onedrive/browse ─────────────────────────────────────
router.get('/browse', requireAuth, requirePermission('ai.search.sharepoint'), async (req: Request, res: Response) => {
  const path = (req.query.path as string) || '/';
  const perms = await getPermSet(req);
  const auth = getAuth(req);

  // Block the top-level request outright if the caller isn't allowed to
  // access this specific folder — prevents "browse to /CEO_Private".
  if (path !== '/' && !isFileAccessAllowed(path, perms)) {
    await logSecurityEvent({
      actorOid: auth?.userId,
      action: 'file.access.denied',
      outcome: 'denied',
      reason: `Denied OneDrive browse to '${path}' — role lacks file access`,
      context: { path },
      req,
    });
  ...
  try {
    ...
    const filtered = filterFileResults(files, perms);
    res.json({ files: filtered, path, filtered_count: files.length - filtered.length });
  } catch (err: any) {
...
// Permission + path-filtered search. Results are scoped to folders the
// user is allowed to see — results from restricted paths are dropped
// silently (not leaked as "filtered out" counts to avoid enumeration).
router.get('/search', requireAuth, requirePermission('ai.search.sharepoint'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const filtered = filterFileResults(files, perms);
    res.json({ files: filtered, query: q });
  } catch (err: any) {
...
// Requires files.upload. The AI-routed destination folder is checked
// against the user's file permissions — if AI tries to route to a folder
// the user can't access, we force it to 'Unassigned'.
router.post('/upload', requireAuth, requirePermission('files.upload'), upload.single('file'), async (req: Request, res: Response) => {
  const auth = getAuth(req);
  const file = req.file;
  ...
  // If the user specified a destination, verify they're allowed to write
  // there. Block otherwise.
  if (targetFolder && !isFileAccessAllowed(`/${targetFolder}`, perms)) {
    await logSecurityEvent({
      actorOid: auth?.userId,
      action: 'file.upload.denied',
      outcome: 'denied',
      reason: `Denied upload to '${targetFolder}' — role lacks access`,
      context: { folder: targetFolder, filename: file.originalname },
      req,
    });
    res.status(403).json({ error: `You do not have access to upload into "${targetFolder}".` });
    return;
  }
...
Return ONLY valid JSON: {"folder": "folder name", "confidence": "high|medium|low", "reason": "brief reason"}`;
  ...
  // Post-AI guard: if AI routed to a folder the user can't access, force
  // to Unassigned. Prevents the "AI suggests you store this in HR folder
  // even though you're a recruiter" scenario.
  if (targetFolder && !isFileAccessAllowed(`/${targetFolder}`, perms)) {
    routingReason = `AI chose '${targetFolder}' but user lacks access — forced to Unassigned`;
    targetFolder = 'Unassigned';
    routingConfidence = 'low';
  }
...
router.get('/uploads', requireAuth, requirePermission('ai.search.sharepoint'), async (req: Request, res: Response) => {
