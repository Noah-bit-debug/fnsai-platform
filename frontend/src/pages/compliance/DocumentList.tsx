...
function statusBadge(status: ComplianceDocument['status']) {
  ...
  return (
    <div style={{ padding: '32px 40px', background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1e293b', margin: 0 }}>Documents</h1>
          <p style={{ fontSize: 14, color: '#64748b', marginTop: 4 }}>
            Manage compliance documents and acknowledgements.
          </p>
        </div>
        <button
          onClick={() => navigate('/compliance/admin/documents/new')}
          style={{
            padding: '9px 18px',
            background: '#2563eb',
            color: '#fff',
      ...
      <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: '#1e293b', marginBottom: 6 }}>No documents yet</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
                {['Title', 'Status', 'Requires Read Ack', 'Roles', 'Created', 'Actions'].map((h) => (
                  <th key={h} style={{
                    padding: '10px 16px',
                    textAlign: 'left',
                    fontSize: 11,
              ...
              {documents.map((doc) => (
                <tr key={doc.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '11px 16px' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1e293b' }}>{doc.title}</div>
                  </td>
                  <td style={{ padding: '11px 16px' }}>{statusBadge(doc.status)}</td>
                  <td style={{ padding: '11px 16px', fontSize: 18, textAlign: 'center' }}>
                    {doc.requires_read_ack ? '✓' : '✗'}
                  </td>
                  <td style={{ padding: '11px 16px', fontSize: 12, color: '#475569', maxWidth: 160 }}>
                    {Array.isArray(doc.applicable_roles) && doc.applicable_roles.length > 0
                      ? doc.applicable_roles.slice(0, 3).join(', ') + (doc.applicable_roles.length > 3 ? ` +${doc.applicable_roles.length - 3}` : '')
                  ...
                  <td style={{ padding: '11px 16px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <Link
                        to={`/compliance/admin/documents/${doc.id}/edit`}
                        style={{
                          padding: '5px 12px',
                          fontSize: 12,
                          fontWeight: 500,
                      ...
                      {doc.status !== 'archived' && (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 6,
                            background: '#fff5f5',
                            cursor: archiving === doc.id ? 'not-allowed' : 'pointer',
                            opacity: archiving === doc.id ? 0.6 : 1,
                            whiteSpace: 'nowrap',
                          }}
                        >
                          {archiving === doc.id ? 'Archiving…' : 'Archive'}
                        </button>
                      )}
                    </div>
                  </td>
